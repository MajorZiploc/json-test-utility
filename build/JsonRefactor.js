...
var JsonRefactor = /** @class */ (function () {
  ...
  JsonRefactor.prototype.isSubsetGeneral = function (subJson, superJson, compareFn) {
    ...
    return subKVA.every(function (subKv) {
      return superKVA.some(function (supKv) {
        return compareFn(subKv, supKv);
      });
    });
  };
  /**
  ...
  JsonRefactor.prototype.isSubsetKeys = function (subJson, superJson) {
    return this.isSubsetGeneral(subJson, superJson, function (sub, sup) {
      return _.isEqual(sub.key, sup.key);
    });
  };
  /**
   *
   ...
   */
  JsonRefactor.prototype.isSubsetSpecialCases = function (
    subJson,
    superJson,
    specialSubKeys,
    specialFns,
    ...
    if (specialSuperKeys === void 0) {
      specialSuperKeys = specialSubKeys;
    }
    if (compareRest === void 0) {
      compareRest = true;
    }
    var specialEntries = _.zipWith(specialSubKeys, specialSuperKeys, specialFns, function (subKey, superKey, fn) {
      return { subKey: subKey, superKey: superKey, fn: fn };
    });
    var didSpecialsPass = specialEntries.every(function (e) {
      return e.fn(subJson[e.subKey], superJson[e.superKey]);
    });
    if (!didSpecialsPass) {
  ...
  JsonRefactor.prototype.subJson = function (json, keys) {
    return this.fromKeyValArray(
      this.toKeyValArray(json).filter(function (kv) {
        return keys.some(function (k) {
          return _.isEqual(k, kv.key);
        });
      })
    );
  };
  ...
  JsonRefactor.prototype.isSubsetArray = function (subArray, superArray) {
    return subArray.every(function (subV) {
      return superArray.some(function (supV) {
        return _.isEqual(subV, supV);
      });
    });
  };
  /**
  ...
  JsonRefactor.prototype.toKeyValArray = function (json) {
    return Object.entries(json).map(function (e) {
  ...
  JsonRefactor.prototype.fromKeyValArray = function (keyValueArray) {
    var _this = this;
    return keyValueArray.reduce(function (acc, ele) {
      return _this.addField(acc, ele.key, ele.value);
    }, {});
  };
  return JsonRefactor;
})();
